...
def audit_feature_group_scales(features: torch.Tensor) -> dict[str, dict[str, float]]:
    """检查各特征组绝对均值/标准差，发现量纲失衡（非 z-score 诊断）。"""
    struct = features[:, :DAY_STRUCT_FEAT_DIM].float()
    out: dict[str, dict[str, float]] = {}
    for name, sl in DAY_FEATURE_SLICES.items():
        if name == "behavior_structure":
            continue
        block = struct[:, sl]
        out[name] = {
            "abs_mean": float(block.abs().mean().item()),
            "std": float(block.std(unbiased=False).item()),
            "max_abs": float(block.abs().max().item()),
        }
    if "behavior_structure" in DAY_FEATURE_SLICES:
        beh_sl = DAY_FEATURE_SLICES["behavior_structure"]
        if beh_sl.stop <= features.shape[1]:
            beh = features[:, beh_sl].float()
            out["behavior_structure"] = {
                "abs_mean": float(beh.abs().mean().item()),
                "std": float(beh.std(unbiased=False).item()),
                "max_abs": float(beh.abs().max().item()),
            }
    return out
...
def run_preflight_audit(
    ...
    *,
    ...
    scale_stats = audit_feature_group_scales(features)
    for name, st in scale_stats.items():
        if st["abs_mean"] < 0.02:
            logger.warning(
                "Preflight %s: abs_mean=%.4f very small — may vanish in recon/encoder",
                name,
                st["abs_mean"],
            )
        if st["max_abs"] > 15.0:
            logger.warning(
                "Preflight %s: max_abs=%.2f large — check group scale constants",
                name,
                st["max_abs"],
            )
    ...
    return {"symbolic": symbolic, "trend": trend_stats, "group_scales": scale_stats}
