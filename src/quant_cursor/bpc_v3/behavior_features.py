...
数据流（归一化分层，避免重复）：
  ...
  L1 窗口内无量纲比（|实体|/振幅、efficiency、rel_vol）→ 结构特征 / K-line token
  L2 vol_context：绝对 close 预计算 log-RV 锚点（lookup_at_anchor，不读 close_Δ）
  L3 纯度标签：固定 SYMBOLIC_THRESHOLDS，无 batch/global z-score
  L4 PrecomputedFeatureComposer LayerNorm（仅 26 维预计算向量）
...
BEHAVIOR_LABEL_SCHEMA = "symbolic_agents_v14_returns_volratio"
...
def symbolic_labels(values: torch.Tensor, low: float, high: float) -> torch.Tensor:
    """固定阈值 3 档 one-hot 标签。"""
