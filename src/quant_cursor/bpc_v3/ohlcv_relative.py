...
def log_volume_change_from_levels(vol: torch.Tensor) -> torch.Tensor:
    """log(V_t) - log(V_{t-1})；首根为 0。"""
    ratio = volume_ratio_from_levels(vol)
    return torch.log(ratio.clamp_min(1e-8))
