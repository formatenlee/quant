...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--use-fine-vq",
        action="store_true",
        help="启用分层残差细码本 VQ（粗码本 + per-coarse 细码本）",
    )
    p.add_argument(
        "--no-codebook-film",
        action="store_true",
    ...
    p.add_argument(
        "--batched-gpu",
        action="store_true",
        help="BatchedGpuDataset：Dataset 直接返回完整 GPU batch，消除 DataLoader 逐样本调度开销。"
        " 隐含 num_workers=0，DataLoader batch_size=1。",
    )
    # 新增：预计算特征物化选项
    p.add_argument(
        "--precompute-features",
        action="store_true",
        default=True,
        help="物化时预计算 24 维特征向量（默认开启，关闭则回退到原始 OHLCV）",
    )
    p.add_argument(
...
def _first_collate(batch: list) -> dict:
    """DataLoader collate for BatchedGpuDataset (batch_size=1, item is already a full batch)."""
    return batch[0]
...
def _resolve_datasets(
    ...
    *,
    ...
    store, train_ds, val_ds, split = build_datasets(
        ...
        precompute_features=args.precompute_features,  # 新增：控制预计算特征
        seed=args.seed,
    )
...
def main() -> int:
    ...
    logger.info(
        "Run dir: %s | instruments=%d | full=%s | epochs=%d | val_ratio=%.2f | seed=%d | day_lb=%d | week_lb=%d | precompute_features=%s",
        out_dir,
        len(instrument_list),
        use_full,
        args.epochs,
    ...
    # 关键改动：BatchedGpuDataset 模式下，训练循环使用 iter_batches() 而非 DataLoader
    if args.batched_gpu and use_cuda:
        logger.info(
            "BatchedGpuDataset mode: contiguous GPU slices, direct iteration (batch_size=%d). "
            "prefetch-factor ignored (num_workers=0).",
            args.batch_size,
        )
        # 确保基础数据集已物化
        ...
        # BatchedGpuDataset 直接迭代，无需 DataLoader
        train_loader = train_ds  # 直接传入 iter_batches()
        val_loader = val_ds
        loader_kwargs = None
        train_shuffle = False
        ...
        if args.preprocessed_dir and not args.gpu_cache_data and args.num_workers > 8:
            logger.warning(
                ...
                "try --num-workers 0 or --batched-gpu if GPU memory allows",
                args.num_workers,
            )
        pin_memory = use_cuda
        loader_kwargs = {
    ...
    logger.info(
        "Device=%s | train_samples=%d | val_samples=%d | batch=%d | workers=%d | materialize=%s | precompute_features=%s | amp=%s",
        device,
        len(train_ds),
        len(val_ds),
        args.batch_size,
        args.num_workers if not args.batched_gpu else 0,
        not args.no_materialize,
        args.precompute_features,
        args.amp,
    )
    logger.info(
        "VQ: codebook_film=%s | fine_vq=%s | num_fine_per_coarse=%d",
    ...
    precompute_purity_thresholds(
        model,
        train_loader if not args.batched_gpu else train_ds,  # BatchedGpuDataset 直接传入
        device=device,
        max_batches=500 if not args.batched_gpu else 50,  # BatchedGpuDataset 批次更大，减少批次数量
    )

    best_val = float("inf")
    non_blocking = use_cuda and not args.batched_gpu and loader_kwargs.get("pin_memory", False) if loader_kwargs else False
    ...
    # 关键改动：BatchedGpuDataset 训练循环适配
    for epoch in range(start_epoch, args.epochs):
        lr = optimizer.param_groups[0]["lr"]
        
        # 训练阶段
        if args.batched_gpu:
            # BatchedGpuDataset 直接迭代，无需 DataLoader 包装
            train_ds.on_epoch_begin()  # 刷新 batch 顺序
            train_metrics = train_epoch(
                model,
                train_ds,  # 传入 BatchedGpuDataset，其 __iter__ 返回 batch dict
                optimizer,
                device=device,
                amp=args.amp,
                non_blocking=False,  # GPU 驻留，无需 H2D
                scaler=scaler,
            )
        else:
            train_metrics = train_epoch(
        ...
        if (epoch + 1) % args.val_every == 0 or epoch == args.epochs - 1:
            ...
            if args.batched_gpu:
                val_ds.on_epoch_begin()  # 验证不需要 shuffle，但保持接口一致
            ...
            if args.vq_adapt_on_val:
                adapt_loader = val_ds if args.batched_gpu else val_loader
                n_steps = adapt_codebook_on_loader(
                    model,
                    adapt_loader,
                    device=device,
                    max_batches=val_max,
                    non_blocking=False if args.batched_gpu else non_blocking,
                )
            ...
            if args.diagnose_on_val and (
                ...
                diag = diagnose_codebook_shift(
                    model,
                    train_loader if not args.batched_gpu else train_ds,
                    val_loader if not args.batched_gpu else val_ds,
                    device,
    ...
    # Token 语义分析
    semantics = model.analyze_token_semantics(
        val_loader if not args.batched_gpu else val_ds, 
        device=device, 
        max_samples=5000
    )
