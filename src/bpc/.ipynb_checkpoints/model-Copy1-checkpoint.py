    ...
    registry.register(
        ScaleConfig(
            ...
            feature_groups=[
                ...
                "attack_proxy",
                "micro_proxy",
                "behavior_structure",  # 新增：8维因果行为代理，打破信息瓶颈
            ],
        )
    )
    registry.register(
...
class CausalFeatureComposer(nn.Module):
    ...
    def _price_structure(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for lag in [1, 2, 5]:
            if log_ret.shape[1] > lag:
                ...
                # 使用 NaN 而非 0，避免引入结构性偏差；后续 nan_to_num 统一处理
                feats.append(torch.full_like(rv, float('nan')))
    ...
    def _behavior_structure(self, x: torch.Tensor) -> torch.Tensor:
        """8维因果行为代理，直接作为编码器输入，打破扩展代理纯度损失的信息瓶颈。"""
        proxies = compute_behavior_proxies_stacked(x)
        proxies = transform_proxies_for_labeling(proxies)
        return proxies  # (B, 8)
    ...
    def extract_raw_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if "behavior_structure" in self.groups:
            group_feats.append(self._behavior_structure(x))
        if not group_feats:
...
class BPCv2(nn.Module):
    def __init__(
        ...
        self.raw_feat_dim = compute_raw_feat_dim(primary_cfg)  # 现在为 24 维
