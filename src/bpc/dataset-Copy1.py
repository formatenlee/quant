...
class MaterializedMultiScaleDataset(Dataset):
    ...
    def __init__(
        ...
        *, 
        ...
        precompute_features: bool = True,  # 新增：预计算 24 维特征
    ):
        ...
        if precompute_features and "day" in self._tensors:
            logger.info("Precomputing 24-dim features (%d samples) ...", n)
            day = self._tensors["day"]  # [n, T, 5]
            feat_chunks = []
            ...
            features = torch.cat(feat_chunks, dim=0)  # [n, 24]
            if share_memory:
                features = features.share_memory_()
            self._features["day_features"] = features  # [n, 24] 预计算特征
            
            # 同时预计算 behavior_proxies（用于 purity loss）
            ...
            self._proxies = proxies  # [n, 8]
        else:
            self._proxies = None

        logger.info(
            "Materialized dataset ready (%d samples, features=%s, proxies=%s)",
            n,
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if "day_features" in self._features:
            sample["day_features"] = self._features["day_features"][idx]  # [24]
        else:
            sample["day"] = self._tensors["day"][idx]  # 回退到原始 OHLCV
        
        # week 保持原始（若 week 也需预计算，可扩展）
...
class GpuCachedDataset(Dataset):
    ...
    def __init__(self, base: MaterializedMultiScaleDataset, device: str = "cuda"):
        ...
        if "day_features" in base._features:
            self._features["day_features"] = base._features["day_features"].to(dev, non_blocking=True)
        else:
...
class BatchedGpuDataset(Dataset):
    ...
    def __init__(
        ...
        if isinstance(base, GpuCachedDataset):
            ...
            # 从 MaterializedMultiScaleDataset 直接上传
            self._features = {}
            if "day_features" in base._features:
                self._features["day_features"] = base._features["day_features"].to(device, non_blocking=True)
