...
class MaterializedMultiScaleDataset(Dataset):
    ...
    def __len__(self) -> int:
        ...
        return self._stock_ids.shape[0]
...
class GpuCachedDataset(Dataset):
    ...
    def __init__(self, base: MaterializedMultiScaleDataset, device: str = "cuda"):
        ...
        self._vol_context = (
            base._vol_context.to(dev, non_blocking=True)
            if getattr(base, "_vol_context", None) is not None
            else None
        )
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self._proxies is not None:
            sample["behavior_proxies"] = self._proxies[idx]
        if getattr(self, "_vol_context", None) is not None:
...
def pin_dataset_share_memory(ds: MaterializedMultiScaleDataset) -> None:
    ...
    if getattr(ds, "_vol_context", None) is not None:
...
class ContiguousBatchDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        if getattr(base, "_vol_context", None) is not None:
            self._field_names.append("vol_context")
            self._field_tensors.append(base._vol_context)
...
class BatchedGpuDataset(Dataset):
    ...
    def __init__(
        ...
        if isinstance(base, GpuCachedDataset):
            ...
            self._vol_context = getattr(base, "_vol_context", None)
            self._proxies_label_ready = getattr(base, "_proxies_label_ready", False)
            self.n = len(base)
        else:
            self._features = {}
            ...
            self._vol_context = (
                base._vol_context.to(device, non_blocking=True)
                if getattr(base, "_vol_context", None) is not None
                else None
            )
            self._proxies_label_ready = getattr(base, "_proxies_label_ready", False)
        ...
        vol_ctx = getattr(self, "_vol_context", None)
        if vol_ctx is not None:
            self._field_names.append("vol_context")
            self._field_tensors.append(vol_ctx)
